...
import type { SkillItem, SessionSkillsConfig } from '../../types/index.ts'
import { ModalDialog, Badge } from '../../components/index.ts'
import { getSkillSourceMeta } from '../../utils/index.ts'

const e = React.createElement

export interface SessionSkillDetailModalProps {
  ...
  onSkillsModeChange: (mode: 'custom') => void
  onClose: () => void
  t: (key: string, vars?: Record<string, string | number>) => string
}

export function SessionSkillDetailModal({
  skill,
  detail,
  skillsConfig,
  isModelDisabled,
  ...
  const handleModalToggleModel = () => {
    if (skillsConfig.mode !== 'custom') {
      onSkillsModeChange('custom')
    }
    onToggleModelInvocable(skill.name)
  }
  ...
  const handleModalToggleUser = () => {
    ...
    onToggleUserInvocable(skill.name)
  }
