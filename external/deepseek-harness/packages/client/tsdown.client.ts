 ...
 * packages/client/tsdown.client.ts (synced 2026-09-10, upstream commit unknown).
...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  const pluginPkgPath = resolvePath(process.cwd(), 'package.json')
  if (existsSync(pluginPkgPath)) {
    const manifest = JSON.parse(readFileSync(pluginPkgPath, 'utf8')) as WorkspaceManifest
    if (manifest.name === id) {
